...
async function adminStartGame(){
  ...
  app.room = updated;
  app.lastRenderedStatus = 'playing';
  render();
}
...
async function tryAdvanceRound(){
  ...
  if(latest.current_round >= latest.total_rounds){
    updated = await updateRoom(app.roomCode, { status:'finished' });
  }else{
    const letter = pickRandomLetter(latest.used_letters);
    const usedLetters = [...(latest.used_letters||[]), letter];
    const roundEndTime = new Date(Date.now() + latest.round_duration*1000).toISOString();
    updated = await updateRoom(app.roomCode, {
      used_letters: usedLetters,
      current_letter: letter,
  ...
  app.room = updated;
  app.players = await fetchPlayers(app.roomCode);
  app.lastRenderedStatus = updated.status;
  app.lastRenderedRound = updated.current_round;
  render();
}
...
async function gameTick(roomRow){
  if(roomRow.status==='playing' && roomRow.round_end_time && Date.now() >= new Date(roomRow.round_end_time).getTime() && !app.busy){
    ...
    setTimeout(async ()=>{
      try{
        const latest = await fetchRoom(app.roomCode);
        if(latest && latest.status==='playing' && latest.current_round===roomRow.current_round){
          await updateRoom(app.roomCode, { status:'judging', judging_by: app.playerId, judging_claimed_at: new Date().toISOString() });
          await new Promise(res=>setTimeout(res, 350));
          const check = await fetchRoom(app.roomCode);
          if(check && check.status==='judging' && check.judging_by===app.playerId){
            app.room = check;
            if(app.lastRenderedStatus!=='judging'){ app.lastRenderedStatus='judging'; render(); }
            await performJudging(check);
          }
        }
  ...
  if(roomRow.status==='judging' && roomRow.judging_claimed_at && Date.now()-new Date(roomRow.judging_claimed_at).getTime()>15000 && !app.busy){
    ...
    setTimeout(async ()=>{
      try{
        const latest = await fetchRoom(app.roomCode);
        if(latest && latest.status==='judging' && latest.judging_claimed_at===roomRow.judging_claimed_at){
          await updateRoom(app.roomCode, { judging_by: app.playerId, judging_claimed_at: new Date().toISOString() });
          await new Promise(res=>setTimeout(res, 350));
          const check = await fetchRoom(app.roomCode);
          if(check && check.status==='judging' && check.judging_by===app.playerId){
            await performJudging(check);
...
async function performJudging(roomRow){
  ...
  if(items.length>0){
    try{
      const { data, error } = await sb.functions.invoke('judge', { body: { letter, items } });
      if(!error && data && Array.isArray(data.results)){
        data.results.forEach(o=>{ if(o && o.valid) validitySet.add(o.id); });
      }else{
        // Edge function unreachable/misconfigured: fall back to accepting
        // anything that at least starts with the right letter, so the
        // game doesn't stall.
        items.forEach(it=>validitySet.add(it.id));
      }
    }catch(e){
      items.forEach(it=>validitySet.add(it.id));
    }
...
function render(){
  ...
  switch(app.room.status){
    ...
    case 'playing': root.innerHTML = renderPlaying(); startPlayTimer(); break;
...
function renderPlaying(){
  ...
  return `
    <div class="sheet"><div class="sheet-inner">
      ...
      <div class="letter-stage"><div class="letter-circle">${letter}</div></div>
      ...
      ${CATS.map(cat=>`
        <div class="field-card">
          ...
          <input type="text" id="inp-${cat}" maxlength="40" placeholder="${letter}... (${CAT_HINTS[cat]})" oninput="onAnswerInput()">
        </div>
      `).join('')}
