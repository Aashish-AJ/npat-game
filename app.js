...
// returns a map "category|word" -> {valid, canonical}
async function fetchCachedWords(pairs){
  ...
  const map = {};
  data.forEach(row=>{ map[`${row.category}|${row.word}`] = { valid: row.valid, canonical: row.canonical || row.word }; });
  return map;
}
// entries: [{category, word, valid, canonical}]
async function cacheWords(entries){
  if(entries.length===0) return;
  const rows = entries.map(e=>({ category:e.category, word:e.word, valid:e.valid, canonical: e.canonical || null, checked_at:new Date().toISOString() }));
...
// Resets this same room back to the lobby -- same code, same players --
// so everyone can play again without re-sharing/re-joining. Scores and
// round history reset; whatever rounds/timer settings are currently set
// carry over (and can still be changed again in the lobby).
async function adminPlayAgain(){
  const updated = await updateRoom(app.roomCode, {
    status: 'lobby',
    current_round: 0,
    used_letters: [],
    current_letter: null,
  ...
  const players = await fetchPlayers(app.roomCode);
  for(const p of players){
    await updatePlayerScore(app.roomCode, p.id, 0);
  }
  app.room = updated || await fetchRoom(app.roomCode);
  app.players = await fetchPlayers(app.roomCode);
  app.lastRenderedStatus = 'lobby';
  app.lastRenderedRound = null;
  render();
}
...
function buildJudgeItems(letter, players, answersByPid){
  ...
  for(const p of players){
    ...
    for(const cat of CATS){
      ...
      const prefixOk = val.length>=2 && letter && val[0].toUpperCase()===letter;
...
async function performJudging(roomRow){
  ...
  const canonicalMap = {}; // id -> canonical (lowercased) spelling, only for valid items
  const toAsk = [];
  items.forEach(it=>{
    ...
    if(cached){
      if(cached.valid){
        validitySet.add(it.id);
        canonicalMap[it.id] = (cached.canonical || it.answer).toLowerCase();
      }
    }else{
      toAsk.push(it);
    }
  ...
  if(toAsk.length>0){
    try{
      ...
      if(!error && data && Array.isArray(data.results)){
        const resultMap = {};
        data.results.forEach(o=>{ if(o) resultMap[o.id] = o; });
        toAsk.forEach(it=>{
          const r = resultMap[it.id];
          const valid = !!(r && r.valid);
          const canonical = (r && r.canonical) ? String(r.canonical).toLowerCase() : it.answer.trim().toLowerCase();
          if(valid){
            validitySet.add(it.id);
            canonicalMap[it.id] = canonical;
          }
          newlyJudged.push({ category: it.category, word: it.answer.trim().toLowerCase(), valid, canonical: valid ? canonical : null });
        });
      }else{
        ...
        toAsk.forEach(it=>{
          validitySet.add(it.id);
          canonicalMap[it.id] = it.answer.trim().toLowerCase();
        });
      }
    }catch(e){
      toAsk.forEach(it=>{
  ...
  for(const cat of CATS){
    ...
    for(const p of players){
      ...
      if(info.valid){
        // Group by the AI's corrected spelling, not the raw text, so e.g.
        // "Zimbabwe" and "Zimbabve" are recognized as the same answer and
        // correctly split points instead of both scoring full marks.
        const norm = canonicalMap[id] || info.text.toLowerCase();
...
function copyRoomCode(){
  ...
  navigator.clipboard.writeText(app.roomCode).then(()=>{
    const el = document.querySelector('.room-code-display');
    const hint = document.getElementById('copy-hint');
    if(el){ el.classList.add('copied'); setTimeout(()=>{ if(el) el.classList.remove('copied'); }, 900); }
    if(hint){
      hint.textContent = 'Copied to clipboard!';
      setTimeout(()=>{ if(hint) hint.textContent = "Tap the code to copy \u00b7 share it with friends to join"; }, 1400);
    }
  }).catch(()=>{});
}

// Lets the admin adjust rounds/timer before the game starts.
async function updateGameSetting(field, value){
  if(!app.isAdmin || !app.room || app.room.status!=='lobby') return;
  const val = parseInt(value, 10);
  const updated = await updateRoom(app.roomCode, { [field]: val });
  if(updated) app.room = updated;
}
...
function renderLobby(){
  ...
  const list = players.map(p=>`
    <li>
      ...
      <span class="score-pill">${p.score||0} pts</span>
    </li>`).join('');
  ...
  return `
    <div class="sheet"><div class="sheet-inner">
      ...
      <p class="code-hint" id="copy-hint">Tap the code to copy &middot; share it with friends to join</p>
      ${app.isAdmin ? `
        <div class="settings-row">
          <div>
            <label>Number of rounds</label>
            <select onchange="updateGameSetting('total_rounds', this.value)">
              ${[3,5,7,10].map(n=>`<option value="${n}" ${n===r.total_rounds?'selected':''}>${n} rounds</option>`).join('')}
            </select>
          </div>
          <div>
            <label>Time per round</label>
            <select onchange="updateGameSetting('round_duration', this.value)">
              ${[30,45,60,90,120].map(s=>`<option value="${s}" ${s===r.round_duration?'selected':''}>${s} sec</option>`).join('')}
            </select>
          </div>
        </div>
        <p class="small-muted center" style="margin-top:-8px; margin-bottom:14px;">Only you (the admin) can change these.</p>
      ` : `
        <p class="small-muted center">${r.total_rounds} rounds &middot; ${r.round_duration}s per round</p>
      `}
      <hr class="divider">
    ...
    <div class="sheet"><div class="sheet-inner">
      ...
      ${CATS.map(cat=>`
        <div class="field-card">
          ...
          <input type="text" id="inp-${cat}" maxlength="40" placeholder="${letter}... (${CAT_HINTS[cat]})" oninput="onAnswerInput()" disabled autocomplete="off" autocapitalize="words" autocorrect="off" spellcheck="false">
        </div>
      `).join('')}
...
function renderFinal(){
  ...
  return `
    <div class="sheet"><div class="sheet-inner">
      ...
      ${app.isAdmin ? `
        <button class="btn gold full" onclick="adminPlayAgain()">Play Again (Same Room)</button>
        <p class="small-muted center" style="margin-top:10px;">Everyone stays in this room &mdash; you'll be able to change rounds/timer again before it starts.</p>
      ` : `<p class="small-muted center">Waiting for the admin to start a new game in this room...</p>`}
      <button class="btn secondary full" style="margin-top:14px;" onclick="location.reload()">Leave &amp; Start Somewhere Else</button>
    </div></div>
  `;
}
